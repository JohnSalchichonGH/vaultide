...
| Each occurrence of an active income template scheduled in M | a flow carries that `(template_id, occurrence_date)`, or a `recurring_template_skips` row does |
...
2. **Income**: one row per due template ("Salary — suggested €2,100 on 25 Sep — Accept · Edit · Skip"). Suggestions dated after today are shown as "upcoming" and cannot be accepted until their date — or through "received today", which records the flow’s financial date as today while keeping the occurrence’s own `occurrence_date`, so the occurrence counts as fulfilled and is never suggested again. A salary scheduled for 1 October and actually received on 30 September is one income entry with `occurrence_date = 2026-10-01` and `received_on = 2026-09-30`: September’s reconciliation correctly sees the cash arrive on the 30th, and October never suggests it a second time. Editing a suggested amount offers "This month only" or "From this month on". Skipping any suggestion writes a `recurring_template_skips` row for that occurrence; for rental income it asks for a reason (vacant / tenant did not pay / other), which is the only occupancy fact the app records.
...
**Bulk history**: grid columns = each position (month-end value, `month_end` precision) and each income template (net); rows = completed months from a chosen start (the current month row is disabled); paste from a spreadsheet (TSV, locale-aware numbers); per-cell validation; batch save in one transaction, aborted entirely on any conflict (20.3).
...
No genuine blockers remain. The blueprint was frozen as v2.1.2 and Phase 0 began from it; it is frozen as v2.1.5 after the corrections in 30.6, 30.7 and 30.8, none of which changed a phase already delivered.

---

## Freeze check

- The seven v2.1.1 defects (30.4), the four v2.1.2 corrections (30.5), the v2.1.3 onboarding-range correction (30.6), the v2.1.4 savings-golden correction (30.7) and the v2.1.5 pre-Phase-3 clarifications (30.8) were corrected and propagated to the schema, algorithms, tests, phases and acceptance criteria; no accounting identity changed except in wording or representation (30.2), and the personal savings rate is an additional derived figure layered on the unchanged tracked identity.
...
- No remaining blocker was found. The blueprint is frozen as v2.1.5.
