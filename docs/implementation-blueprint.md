...
| F15 | Monthly average FX rate (§7.6) is undefined. | Low | Arithmetic mean of the stored daily rates in the calendar month, from one observation upwards; with none, `Unavailable` — there is no completed-month fallback, because the dated lookup’s ten-day window lies inside the month (30.17). The current month through `D` keeps its sparse-data fallback (10.2). |
...
- **Financial write-coordination clarification (v2.1.18 → v2.1.19):** 15.3 named "historical correction" without saying which edits are one, 20.3 had optimistic versions and two row locks but no rule about when a financial decision may be read, and 6.3 let a financial delete carry no version at all (30.22). Historical correction is now an update or delete of a source fact whose financial period is completed on either side, plus a dormancy transition whose dated episode reaches completed history; every mutation of mutable financial evidence takes one per-user, transaction-scoped write mutex before its first authoritative read and performs its validation, decisions, writes and audit inside that transaction; `categories.archived_at` is named a reference dependency, locked `FOR SHARE` by the financial write rather than serialized on the mutex; a valuation, income, expense or transfer delete carries the version the client rendered; `WRITE_BUSY` joins the error taxonomy; and 21.5 loses a historical-correction restore no phase can yet run. No identity, status, trigger, availability rule or schema changes, and no migration.
- **Corrective-action UX clarification (v2.1.17 → v2.1.18):** 8.5 named a suggested action for every issue and 15.3 asked for "one-click actions", without saying what a click may do or where the action lives, and four of those suggestions described a weaker interaction than the delivered product can give (30.21). A click now enters a focused corrective interaction and never writes a financial fact by itself; "Accept as adjustment" becomes an operation whose amount, date, settlement and null cash leg the server derives from the discrepancy it recomputes; the cross-currency suggestion prefills the two native residuals and chooses neither account nor date even when one account is the only candidate; `first_balance` offers the earlier month-end balance through the existing single-month editor instead of waiting for the bulk editor; `flow_without_cash_account` carries the identity of the record that has no account, because under its own trigger there is no account to choose yet; and a correction launched from a current-month issue stays inside `[start(M), D]`. No identity, status, trigger, availability rule or schema changes.
- **Reconciliation-evidence correction (v2.1.16 → v2.1.17):** two ways a delivered engine turned an absence of evidence into a confident figure (30.20). 8.1 made `dormant_zero` depend on `is_dormant` alone, and that flag is present-tense: marking an account dormant today rewrote every earlier month lacking a statement as a zero close, which could make a month `reliable` with its whole former balance reported as spending, remove the honest span across it, and change its completeness. The current dormant episode is now dated — `cash_accounts.dormant_from`, the date of the zero balance that justified it — and a structural zero is permitted only on or after that date; an episode may begin only on a zero balance that no attributed flow post-dates. And 8.3 took the worst of no bucket statuses to be `reliable`, so a completed month in which no cash account took part reported an available zero and entered rolling averages as an observation. Such a month is now `unavailable`, its tracked figures are unavailable rather than zero and name no missing currency, and the two untracked settlements stay computable. No identity changes; one nullable column is added.
- **Server-authoritative Monthly financial recomputation correction (v2.1.15 → v2.1.16):** the Phase 3 roadmap asked for a "client-side preview", 4.2 had the monthly editor import the `finance` reconciliation engine to recompute the reconciliation panel as the user types, and 15.3 recomputed that panel client-side on every change and autosaved every field on blur. Read literally, the browser becomes a second financial calculation path, and a multi-field aggregate saved one field at a time passes through invalid financial states (30.19). Authoritative financial figures are now recomputed through the server's `application` and `finance` path once a mutation commits, and the page refreshes from that result; the browser validates draft input and shows pending and error state but calculates no figure; a simple field keeps its autosave, and a multi-field financial aggregate may save atomically with an explicit Save. No identity, schema or delivered phase changes.
...
- **Feedback:** the browser validates draft input and shows pending and error state; financial figures are recomputed only through the `application` and `finance` path after a mutation commits, and the page refreshes from that result (30.19).
...
**`cash_accounts`** — `position_id uuid PRIMARY KEY`, own, `kind position_kind NOT NULL DEFAULT 'cash' CHECK (kind = 'cash')`, FK `(position_id, user_id, kind) → positions(id, user_id, kind)`; `account_type cash_account_type NOT NULL` (`checking, savings, cash, brokerage_cash, other`), `institution text NULL`, `is_dormant boolean NOT NULL DEFAULT false` (the present-state flag), `dormant_from date NULL` (the date the **current** dormant episode is known zero from: the `valued_on` of the zero balance that justified it, never the day the flag was set; 8.8, 30.20), `CHECK (is_dormant = (dormant_from IS NOT NULL))`. Dormancy is settable only while the latest balance is exactly 0 and no attributed flow is dated after that balance; the service clears both columns together when a non-zero balance or an attributed flow is recorded, or when the balance at `dormant_from` is deleted or re-dated. Audit: with position.
...
- **Closing state** `close(a, M)`: `month_end` if a month-end balance exists; `closed_zero` if `closed_on ∈ M` (balance 0 by definition); `dormant_zero` if the account's current dormant episode covers `end(M)` — `dormant_from ≤ end(M)` (8.8, 30.20) — and no month-end balance exists (carried 0); `carried` if the latest valuation ≤ `end(M)` exists but is not a month-end balance for M; `missing` if no valuation ≤ `end(M)` exists.
...
reconcileMonth(user, M):                     // M completed
  ...
  emit MonthReconciliation { M, buckets[], monthStatus = buckets is empty ? 'unavailable' : worst(bucket statuses), completeness (12.6) }   // 30.20
```
...
Month status = worst across buckets (`unavailable > unresolved > provisional > estimated > reliable`); the reporting-currency total is `partial` when any bucket is `unavailable`. A completed month with **no bucket at all** — no cash position participates in M and no null-leg flow is dated in it — is `unavailable`: no tracked-cash interval was observed, and the worst of no statuses must not read as `reliable` (8.6 rules the same for an empty month-to-date inclusion set; 30.20). An account carried as `dormant_zero` never affects status but is listed in the metadata ("dormant, carried at 0").
...
| `first_balance` | pre-existing account first tracked in M | info | Enter an earlier month-end balance if known: one balance through the previous month's Accounts editor, many through the bulk editor when it exists (30.21) |
| `flow_without_cash_account` | null-leg flow in a currency with no participating cash account. The issue names the record it is about — its kind, its id and its financial date (30.21) | blocking | Add a cash account of that currency, since under this trigger none takes part and there is none to choose yet; then review the named record, whose null leg becomes supported and may optionally be attributed through its own row editor (30.21) |
| `unexplained_inflow` | `unclassified < 0`. Variant A when `total < 0` ("cash grew more than your records explain"); variant B when `total ≥ 0` ("known expenses exceed the cash that left; an inflow may be missing, or an expense was paid from outside tracked cash"). The trigger already implies `ΣK > total`, so that comparison cannot separate the two (30.11); the sign of the tracked total does, with exactly zero on variant B’s side because a tracked total of zero is not cash growth. The amount reported is `−unclassified`, in both variants | blocking | Add income; Add withdrawal/loan proceeds/asset sale; Add transfer from another currency; Correct a balance; Mark an expense as paid from an untracked source; and, as the fallback when none of those is the answer, Accept as adjustment — one `income_entries.kind = adjustment` row for the **currently** unexplained amount, `tracked_cash`, with no cash account and dated the interval's endpoint (30.21). The order the actions are offered in follows the variant |
| `possible_missing_conversion` | a missing cross-currency transfer’s own signature (30.15): bucket C1 has `unexplained_inflow` `X = −unclassified`, and another native bucket C2 of the same month has a computed **positive** `unclassified` `U2` — the spending spike — with `X2 ≤ U2 ≤ 1.05 × X2`, where `X2` is `X` converted C1 → C2 at the month’s average rate. C1 is the destination and C2 the source; every qualifying C2 is one candidate under a single advisory. Completed months only | advisory | Link as cross-currency transfer, prefilled with the two native residuals alone: the date and **both** accounts stay the user's, even where one account is the only candidate of its currency, and no fee is assumed (30.21) |
| `possible_missing_interest` | per-account residual on a savings/brokerage_cash account is positive and < 0.5 % of its balance | advisory | Record interest income, prefilled with the account the residual belongs to and that residual as a suggested amount; the date stays the user's (30.21) |
...
| `large_unclassified` | `unclassified > 2 ×` the median `unclassified` of the `reliable` months among the six calendar months `M−6 … M−1`, in the same native currency and with at least three such months (30.15) | advisory | Add known expenses |
| `stale_investment` / `stale_property` | age > threshold | advisory (completeness only) | Update valuation; Keep |

Each action above is offered where the issue is shown, and **one click enters that correction**: a focused form carrying what the issue already knows, the existing editor for the record, or the month and row that maintain it. Nothing is written until the user confirms what will be recorded, and an action whose record type the current phase cannot represent is not offered at all (30.21).

Dismissing an advisory issue writes its key to `month_reviews.dismissed_issues`. Blocking issues are resolved by data or left open (`unresolved`); nothing stores that an issue was handled, and a corrective action never dismisses an advisory on the user's behalf (30.21).
...
- **Dormant account**: carried at 0 without confirmation, from `dormant_from` onward (below); any non-zero balance or attributed flow clears `is_dormant` and `dormant_from` together, and the month then needs a month-end balance. An **attributed** flow is a tracked-cash income or expense whose cash position is that account, or a transfer with that account on either side; a null-leg flow attributes to no account and clears nothing. The clear happens inside the flow’s own transaction and is audited, and it applies to a back-dated flow as well — the conservative direction, because clearing only ever asks for more evidence, whereas keeping the flag would let an assumption make a month look reliable. Deleting the last attributed flow does **not** restore dormancy: dormancy is a user assertion, re-made only through the explicit action, which requires the latest balance to be exactly zero and no attributed flow dated **after** that balance. A flow dated the same day does not make the zero stale, because a valuation dated `d` reflects every flow dated `≤ d` (8.1). **The flag is not historical evidence.** `is_dormant` says the account is dormant now; `dormant_from` — the `valued_on` of the zero balance that justified the transition, never the day the user set the flag — says from when. At any date `d`, whether a month end (8.1, 8.7) or the month-to-date date `D` (8.6), a structural zero is permitted only when the current episode covers it, `dormant_from ≤ d`, and the latest balance on or before `d` exists, is dated on or after `dormant_from`, and is exactly zero. A missing balance, a non-zero balance, or a balance dated before `dormant_from` — a zero from before the episode is not evidence about it — falls back to the ordinary `carried`/`missing` rules. Before `dormant_from` those rules apply as well, exactly as for an account that is not dormant. Only the current episode is stored: when the account wakes the anchor disappears, an ended episode is not remembered, and its months fall back to their own evidence — a statement, a confirmation, or a span across them — which prefers an unavailable month to an invented zero. Deleting or re-dating the balance at `dormant_from` wakes the account like any other conflicting record; no other zero is searched for, and the user may mark it dormant again (30.20).
...
| Monthly average for a **completed** M | Arithmetic mean of the stored daily rates with `rate_date ∈ M`, from **one** observation upwards, `approximate = false`; with none, `Unavailable('fx_missing')`. There is no fallback outside M: `rateOn(end(M))`’s ten-day lookback lies entirely inside M, so it could only find what the month was already known not to have (30.17). A completed average is therefore never `approximate`. |
...
| Item | Satisfied when |
|---|---|
| Each active cash account not dormant at `end(M)` (8.8) | closing `month_end`/`closed_zero` |
...
| Spending | Tracked + additional | per month: tracked spending (consumption, known/unclassified, costs), **additional spending** (paid by me outside tracked accounts), **total spending** (tracked + additional), **paid by others** (informational, outside totals); statuses; spans; saved from income and savings rate (12.5); rolling 3/6/12 (tracked spending only, over the N calendar months ending at M, counting a month only when it is `reliable` and its reporting-currency total is complete — 30.15); categories; largest known | Add known expense with how it was paid (tracked account / by me outside tracked accounts / by someone else), open month | Stacked bars (known/unclassified) with status hatching; span bars across gaps; additional spending as a separate series; paid-by-others as a muted memo series; category bars; table | "Enter two month-end balances to see inferred spending" | Unavailable months as gaps with reason and span links; provisional current month hatched or "update all accounts" note | bars scroll |
...
8. **Reconciliation**: per currency: the identity with real numbers, status, issues with one-click actions, per-account residuals; for the current month the provisional MTD identity "through <D>", with the newer-balances note or the no-common-date message, each offering the "Update all today" action. A click enters the correction — a prefilled form, the existing editor, or the exact row — and the record is written only when the user confirms it (8.5, 30.21); a correction offered for the current month stays inside `[start(M), D]`, because nothing dated later is in the figure it would fix. Recomputed by the server after every committed change; the server's result replaces it after save (30.19).

A simple field autosaves on blur with optimistic UI and version checks; a multi-field financial aggregate that would pass through an invalid financial state if saved field by field is submitted atomically with an explicit Save (30.19). Keyboard: Tab/Enter navigation, `[`/`]` previous/next month.
...
**Historical correction**: an **update or delete of a source financial fact whose financial period is completed on either its before or its after side**, and a dormancy transition whose dated episode reaches completed history (8.8). Such an edit opens the inline editor with a confirmation showing before → after, the affected months and spans, and what changes structurally — a span appearing or disappearing, a status, a completeness, an issue arriving or clearing; reason optional; audit written. A single historical **creation** is outside this ceremony on first-assertion-versus-revision grounds rather than for want of historical consequence: there is no before-image to show, and the corrective actions of 30.21 already create records inside a completed month. Bulk history below uses the same confirmation whatever mix of creates, updates and clears a batch carries, because there the aggregate effect of the batch is the thing to see. Every one of these is one atomic per-user financial write (20.3). **Built**: the editors ask the server before every save, and a qualifying edit goes through Review changes → Confirm correction; a save that reaches the services directly is refused rather than applied.
...
- A historical correction confirms inside that same transaction, comparing the impact fingerprint it showed against the one it recomputes there (30.22); its preview reads one coherent `repeatable read`, `read only` transaction, takes no write mutex and blocks nothing.
...
- **Frontend:** monthly editor (15.3 sections 1–4, 8) with autosave for simple fields, an explicit atomic Save where a multi-field financial aggregate needs one, browser-side input validation, and server-authoritative recomputation after each committed mutation (30.19); Spending page; Income pages; bulk grid with paste.
...
### 30.15 v2.1.12 — cash-flow and savings availability corrections

12.5 stated when a savings figure is unavailable but left `TotalSpending` out of
the list, offered a savings rate two different shapes, and left two 8.5
advisories and the Phase-3 rolling window without a single meaning. None of this
touches an identity, the schema, spans, or a phase already delivered; each item
below is what an engine must do.

1. **A total spending figure exists only when tracked spending is a spending
   figure.** `TotalSpending = TrackedTotalSpending + AdditionalSpending` is exact
   in a `reliable` bucket, computed and marked `estimated` in an `estimated` one,
   and computed through `D` and labelled provisional in the current month. It is
   `Unavailable` in an `unresolved` bucket, `Unavailable` whenever
   `TrackedTotalSpending` is absent, and `Unavailable` in a current month with no
   `D`. There is no zero fallback in any of those cases.

   In an `unresolved` bucket a negative `unclassified` cannot be read as negative
   spending (R6), and the arithmetic sum is not a smaller answer but a wrong one.
   With `TrackedTotalSpending = −1,291`, `ΣK = 411`, `unclassified = −1,702` and
   `AdditionalSpending = 50`, no figure of `−1,241` is produced; with
   `TrackedTotalSpending = 100`, `ΣK = 500`, `unclassified = −400` and
   `AdditionalSpending = 50`, no figure of `150` is produced either, because the
   known expenses alone already establish a lower bound of 500. What the bucket
   keeps is what it measured: `TrackedTotalSpending` as arithmetic evidence,
   `knownTrackedExpenses`, the unexplained-inflow amount, and 8.4’s "spending ≥
   known" presentation, which needs no stored lower-bound field because the known
   total is exact in every status (30.12). `AdditionalSpending` stays
   independently exact throughout.

2. **An aggregate savings rate is never `Partial`.** The reporting-currency
   `SavingsRate` exists only when the aggregate `PersonalSavings` is complete, the
   aggregate `ExternalIncome` is complete, every conversion both aggregates need
   exists, and `ExternalIncome ≠ 0`; then it is the exact unrounded ratio.
   Otherwise it is `Unavailable` with a reason. Neither a quotient of two partial
   aggregates nor a complete known part over an incomplete total may be presented
   as the user’s savings rate: 7.6’s `Partial` is defined for aggregations over a
   list and remains in force for every additive money figure, but a ratio of
   partial sums is not the ratio of the totals. A rate computed over one complete
   native bucket is an exact scalar rather than a partial aggregate, and stays
   available when it is labelled with its currency — a missing USD bucket should
   not hide what is known about EUR.

3. **12.5’s tracked inputs use 8.1’s flow scope, not a second scan.** Every
   tracked-cash quantity in 12.5 classifies exactly the legs the reconciliation
   bucket already scoped: same currency, same interval, same explicit attribution,
   participation, `first_balance` exclusion and null-leg support; for the current
   month, `[start(M), D]` with nothing dated after `D` reaching `ExternalIncome`,
   the cost decomposition, `TrackedSavingsFromIncome`, `TotalSpending`,
   `PersonalSavings` or `SavingsRate`. A tracked income or `K` expense attributed
...
4. **`large_unclassified` has one window and one median.** For a completed month
   ...
   `unclassified ≥ 0`, which admits a `reliable` or `estimated` bucket and
   excludes `unavailable`, `unresolved` and the provisional current month. The
   baseline is the six **calendar** months `M−6 … M−1` — never a search further
   back for six observations — from which only buckets of `C` whose status is
   exactly `reliable` contribute; `estimated`, `unresolved`, `unavailable`,
   provisional months and spans do not. The asymmetry is deliberate and worth
   naming: an `estimated` current month may be compared against a baseline that
   admits no `estimated` month, because the trigger needs a computed number while
   the baseline must be trustworthy. Fewer than **three** contributing months and
   there is no advisory. The median is taken over the exact `unclassified`
   `Decimal`s — the middle value for an odd count, the exact arithmetic mean of the
   two middle values for an even one, with no rounding — and the trigger is
   strictly `unclassified_M > 2 × median`. A median of exactly zero is not a
   special case: with three contributing months, any positive `unclassified`
   clears it, and no absolute floor is invented to soften that. Native currency
   only, no FX, no comparison of one currency’s history against another’s, and
   completed months only.

5. **Phase-3 rolling windows are `TrackedTotalSpending`, over calendar months.**
   Phase 3 rolls that one series; 15.5’s full cash-flow surface is Phase 8. For
   `N ∈ {3, 6, 12}` and a completed display month `M` the window is exactly the
   `N` calendar months `M−(N−1) … M`. A month that does not qualify consumes its
   calendar position and is not replaced by reaching further back — a three-month
   average must not silently become a nine-month lookback. A month qualifies only
   when its overall month status (8.4’s worst across buckets) is `reliable`, which
   excludes `estimated`, `unresolved`, `unavailable`, the provisional current month
   and span-reconstructed months; "reliable months only" is authoritative and
   15.5’s looser "unavailable months excluded" is corrected accordingly.
   Reliability alone is not enough for a reporting-currency observation: if the
   month’s reporting-currency `TrackedTotalSpending` is `Partial` or `Unavailable`
   because a native bucket could not be converted, that month contributes no
   observation, since averaging a partial monetary series states a number nobody
   measured. That is a series-eligibility rule and does not change the month’s
   native reconciliation status. The average is the arithmetic mean of the
   surviving observations, unweighted and unrounded; with none it is absent, with
   one or more it is computed, and the contributing observation count travels with
   it so a three-month window holding one observation can say so. No minimum count
   is imposed. `is_one_off` does not alter this series — that exclusion belongs to
   13.6’s projection baseline. Spans never contribute and are never divided across
   their months, and month-to-date never contributes (8.6). A completed month
   with no bucket is `unavailable` and its reporting `TrackedTotalSpending` is
   `Unavailable` (30.20), so it contributes no observation and the months before
   a user began tracking are not averaged in as zeros; a month that was observed
   and reconciled to a `reliable` zero is an observation like any other.

6. **`possible_missing_conversion` is the missing transfer’s own algebraic
   signature.** An unrecorded cross-currency transfer leaves cash in the
   destination currency with no `Nin` recorded, so its `unclassified` goes
   negative and it raises `unexplained_inflow` with amount `X = −unclassified`;
   and it removes cash from the source currency with no `Nout` recorded, so that
   bucket’s `unclassified` — the "spending spike", which 7.4 and 12.3 both call
   inferred spending — goes positive. So, for completed months only: take a bucket
   C1 with `unexplained_inflow X > 0` as the candidate destination, and every
   other native bucket C2 of the same month with a computed `U2 = unclassified > 0`
   as a candidate source. An `unavailable` or `unresolved` C2 cannot qualify,
   having no positive computed residual; `reliable` and `estimated` may. Convert
   `X` from C1 to C2 at `M`’s monthly-average rate to obtain `X2`; if that lookup
   is `Unavailable`, C2 is not a candidate.

7. **The band keeps both halves of the original phrase.** The match is
   `X2 ≤ U2 ≤ 1.05 × X2`, comparing exact `Decimal`s with no display rounding —
   a floor at `X2` and 5 % of headroom above it, not a symmetric ±5 % band. `U2`
   below `X2` does not qualify however close it is. This is the ordinary shape of
   a real conversion: the source outflow is at least the destination inflow valued
   at the market monthly average, with spread and friction accounting for the
   difference.

8. **A completed monthly average is either available or absent; it is never
   approximate.** *(Corrected in v2.1.14; see 30.17.)* v2.1.12 wrote here that an
   available monthly average marked `approximate` still supported the advisory
   and that a candidate had to carry that flag. No such average exists: the
   advisory is completed-month-only, and 10.2's completed average is the mean of
   one or more in-month observations or `Unavailable`, with no fallback outside
   the month. So when the completed monthly-average lookup for M is available it
   rests on at least one observation inside M and the candidate may be
   evaluated; when it is `Unavailable`, C2 is not a candidate. The 5 % headroom,
   the advisory class and the user's confirmation of the prefilled transfer are
   unchanged.

9. **Several source currencies are ordered candidates under one advisory.**
   `month_reviews.dismissed_issues` stores issue **keys**, so three possible source
   currencies must not become three indistinguishable
   `possible_missing_conversion` objects for the same destination bucket. One
   advisory is emitted for the unexplained-inflow bucket, carrying every
   qualifying C2 once, ordered by source currency code ascending. A candidate
   carries what a later action needs: source and destination currencies, the source
   amount `U2`, the destination amount `X`, the converted comparison amount `X2`,
   the rate it used (30.17). Prefilling a transfer
   uses the two native residuals `U2` and `X`, never `X2` — the monthly average is
   evidence for the suggestion, not a claim about the bank’s rate — and no cash
   account is chosen automatically when more than one exists. The issue key stays
   `possible_missing_conversion`; no per-pair key such as
   `possible_missing_conversion_USD_EUR` is created, because dismissal is
   deliberately key-based and the keys are stable. Where several destination
   buckets independently carry unexplained inflows, each may hold its own advisory
   with its own candidate list.

10. **The cost buckets partition `ΣK`; they never add to it.** For a bucket whose
    spending arithmetic is valid,
    `KnownConsumption + PropertyOperatingCosts + InterestAndFees + TransactionCosts
    + ExternalOutflows = ΣK`, and `Consumption = KnownConsumption + Unclassified`,
    so `Consumption + PropertyOperatingCosts + InterestAndFees + TransactionCosts +
    ExternalOutflows = TrackedTotalSpending`. This already follows from 12.5, and is
    stated because a later phase will supply cost classifications from sources that
    are not expense rows — a mortgage payment’s interest, for one. Such a fact
    **classifies a portion of `ΣK`**; adding it on top would create money and break
    the identity. An engine given classifications exceeding the known tracked
    expense total must fail rather than invent the difference, and the consumption
...
2. **A completed month's residual uses that month's average.** The arithmetic
   mean of the stored daily rates dated inside M, from one observation upwards.
   v2.1.13 added "with none at all, `rateOn(end(M))` marked `approximate`"; 30.17
   removes that as unreachable — a completed month with no observation in it is
   `Unavailable`.
...
### 30.17 v2.1.14 — completed monthly-average reachability correction

Written before `possible_missing_conversion` is implemented, because that
advisory is the first consumer of a completed month's average rate other than
the residual conversion. One statement in v2.1.13 described a state the accepted
lookup rules cannot produce. Nothing below changes an identity, a native figure,
a rate that has ever been returned, the schema, or a phase already delivered.

1. **What v2.1.13 still said.** F15, 10.2 and 30.16 item 2 gave a completed
   month with no stored rate observation a fallback: `rateOn(end(M))`, marked
   `approximate`. 30.15 items 8 and 9 then built on it — an approximate
   completed average could support a `possible_missing_conversion` candidate,
   and the candidate had to carry that its rate was approximate.

2. **Why that state is unreachable.** `rateOn(d)` takes the greatest stored
   `rate_date ≤ d` and refuses anything more than ten calendar days back
   (10.2). Every month has at least twenty-eight days, so
   `end(M) − 10 days ≥ start(M)` for every M, and every date `rateOn(end(M))`
   may reach lies inside M. The completed average falls back only after finding
   **no** observation dated inside M, so the fallback searches a window already
   known to be empty and returns `Unavailable`. An available completed average
   marked `approximate` has never existed and cannot.

3. **One observation is an average.** A completed month with one, two, three or
   four in-month observations was never "too thin": its minimum has always been
   one, their arithmetic mean is returned, and it is `approximate = false`. Only
   the current month through `D` has a five-observation threshold.

4. **None is unavailable.** A completed month with no observation in M yields
   `Unavailable('fx_missing')`. No rate from before the month, no previous
   month's close, no nearest rate, no wider lookback and no provider fetch
   stands in for it; 10.5 applies, and the figure that needed it is partial.

5. **The current month keeps its fallback.** Through `D`, fewer than five
   observations — zero included — still fall back to `rateOn(D)` marked
   `approximate` (30.16 items 3–5). That lookback is anchored on `D` and may
   legitimately reach a rate from the last days of the previous month when `D`
   is early; it is a sparse-data fallback for a figure still being measured, not
   a completed average, and it is why `approximate` stays in the rate
   vocabulary.

6. **`estimatedConversion` is untouched and distinct.** A completed residual
   converted at its month's average is still `estimatedConversion`, because the
   amount is presented through an average convention rather than a dated
   realized rate (8.11, 10.3). That average is not approximate. `exact = false`
   on a dated weekend or holiday lookup is a third, unrelated marker. Rolling
   eligibility (30.15 item 5, 30.16 item 11) is unchanged.

7. **`possible_missing_conversion` has no approximate-completed branch.** The
   advisory is completed-month-only (30.15 item 6). Its comparison rate is the
   corrected completed average: available — resting on at least one in-month
   observation — and the candidate is evaluated, or `Unavailable` and C2 is not
   a candidate. 30.15 item 8 is corrected accordingly, and the candidate no
   longer carries whether its rate was approximate, since it never can be. The
   band `X2 ≤ U2 ≤ 1.05 × X2`, the ascending candidate order, the prefill from
   `U2` and `X` rather than `X2`, the refusal to choose a cash account when more
   than one exists, and the single stable issue key are unchanged.

8. **Spans.** A span's average is the day-weighted average of its constituent
   completed months' averages (10.2). With any constituent average
   `Unavailable`, the span's is `Unavailable`; a span average is therefore never
   `approximate` either. No span rule changes.

9. **No implementation changed.** The accepted engine already behaves this way:
   the shared fallback branch is dead for a completed month, one observation is
   averaged, none is unavailable, and every test that pins an `approximate`
   average is a current-month case through `D`. This section makes the text say
...
6. **Which phase counts which row.** 12.6's table stays the one end-state
   ...
   - every cash account that participates in M under 8.1 and is not dormant at
     `end(M)` — its dormant episode, if it has one, does not cover that date
     (8.8, 30.20). An account dormant at `end(M)` is outside the count, not a
     satisfied item;
...
### 30.19 v2.1.16 — server-authoritative Monthly financial recomputation correction

Written before Monthly transfers are implemented: the transfer editor is the
first Phase 3 Monthly editor that changes several financial fields of one fact
at once. Phase 3 asked for a client-side preview without saying what a preview
is. The product owner ruled on it; this section records the ruling. Nothing
below changes an identity, a reconciliation figure or status, the schema, or a
phase already delivered.

1. **What was underspecified.** The Phase 3 roadmap asked for a monthly editor
   "with autosave and client-side preview" and did not say whether the preview
   meant ordinary feedback on a draft or an independent recalculation of the
   month's financial results in the browser. 4.2 and 15.3 item 8 read it the
   second way: the editor would import the `finance` reconciliation engine and
   recompute the reconciliation panel as the user types, and the server's result
   would replace it after save. Built literally, that is two financial
   calculation paths for one set of facts. 15.3 also had every field autosave on
   blur, which a fact whose fields are valid only together cannot do without
   committing invalid states in between.

2. **Ruling.** Authoritative financial results — reconciliation, spending,
   savings, availability and status, and the figures derived from them — are
   computed through the existing server path: `application` loads the evidence
   and `finance` calculates. After a financial mutation commits, the page
   refreshes from that result. Nothing calculated in the browser stands in for
   it, however briefly.

3. **What stays in the browser.** Client code may validate the shape of what is
   typed, apply the field constraints it already knows for immediate feedback
   (20.1's client layer: a currency's minor units, a date not after today), keep
   an unsaved draft, and show pending, conflict and error state. It does not
   become a finance engine and computes no reconciliation, spending, savings or
   status figure of its own. Server validation stays authoritative whatever the
   browser has already checked (20.1).

4. **Autosave and an aggregate Save.** A simple field editor may autosave on
   blur with its version check (15.3, 20.3). A multi-field financial aggregate
   may instead be submitted atomically with an explicit Save, where saving its
   fields one at a time would commit invalid intermediate financial states.
   Monthly's cash-transfer editor is the first Phase 3 use of that rule. It is a
   rule for such an aggregate, not a retreat from autosave elsewhere.

5. **Why.** A browser copy of the engines would duplicate financial logic that
   already has one tested home, and the two copies could diverge whenever either
   changed. It would need those engines and the raw evidence they read —
   balances, flows and rates — shipped to the browser only to draw a figure the
   server then replaces. And it would put a speculative figure on screen where
   this product shows authoritative ones.

6. **What does not change.** No identity, reconciliation rule, status or issue,
   financial-date rule, FX rule, schema or server-side validation changes, and
   no accepted phase does. The Monthly sections built so far already refresh
   from the server after a save rather than recalculating in the browser; 4.2,
   15.3 and the Phase 3 roadmap now say so. What `apps/web` may import (19) is
   unchanged.

---

### 30.20 v2.1.17 — reconciliation-evidence correction

Written before the standalone Spending page, which is the first reader of the
completed-month history and its rolling averages. A cold audit found two places
where a delivered engine turned an absence of evidence into a confident figure.
Both follow the blueprint's own wording literally, so the wording is what is
corrected. No identity changes. One nullable column is added to
`cash_accounts`; historical acceptance records are left as they were written.

**The current dormant episode is dated**

1. **The flag is present-tense, and 8.1 read it as history.** 8.1 made
   `dormant_zero` depend on `is_dormant` alone. An account that held 5,000 in
   January, has no February statement, was emptied in March and is dormant today
   therefore closed February at zero: February became `reliable` with 5,000 of
   unclassified spending nobody spent, March became `unresolved` by the same
   amount, the honest February–March span disappeared because February's end
   had become a complete anchor, and February's completeness changed. R22 and
   D32 meant the opposite — "a non-zero account can never silently make a month
   reliable".

2. **No rule over the rows a historical read holds can repair that.** A month
   whose account was dormant throughout and a month that precedes a
   reactivation show identical evidence on or before their own end; they differ
   only in what was recorded afterwards, which a read of that month does not
   load and must not depend on. The missing fact is temporal, so it is stored.

3. **`dormant_from` is the zero-evidence date.** `cash_accounts.dormant_from
   date NULL` is the `valued_on` of the zero balance that justified the current
   dormant transition. It is not the day the user set the flag: an account at
   zero since 31 January and marked dormant on 18 September is dormant from 31
   January, so February to August need no statement. `is_dormant` stays as the
   present-state flag every present-tense reader already uses, and
   `CHECK (is_dormant = (dormant_from IS NOT NULL))` keeps the two from
   disagreeing.

4. **A structural zero needs the episode to cover the date.** At any date `d` —
   a month end, a span anchor or the month-to-date date `D` — `dormant_zero` is
   permitted only when `dormant_from ≤ d` and the latest balance on or before
   `d` exists, is dated on or after `dormant_from`, and is exactly zero. Before
   `dormant_from` the ordinary `carried`/`missing` rules apply, exactly as for
   an account that is not dormant, and completeness counts the account as a
   requirement. They apply as well where that latest balance is missing, is
   non-zero, or is dated before `dormant_from`: a zero from before the current
   episode says nothing about it, because money may have moved through the
   account in between — which is what the start date records. The write rules
   below leave no such state standing, but a balance is written before the wake
   that follows it, so a read in between, or a defect on a write path, can meet
   one; an engine that does must ask for evidence rather than supply a zero.

5. **Starting an episode.** An account may be marked dormant only when its
   latest balance on or before today exists, is exactly zero, and no attributed
   flow (8.8) is dated after it. The old check looked at the balance alone, so a
   zero from January still authorized dormancy after money had moved through
   the account in February. A flow dated the same day as the balance does not
   make it stale (8.1's same-day rule). The check and the transition are one
   transaction over the locked account, so no flow can arrive between them. An
   edit that leaves an already-dormant account dormant does not move the date.

6. **Ending one.** Every wake clears `is_dormant` and `dormant_from` together:
   a non-zero balance, an attributed flow on either side of any date — the
   conservative direction 8.8 already takes — the user clearing the flag, and
   now also deleting or re-dating the balance at `dormant_from`, because a
   correction must not leave the episode resting on a record that is gone. No
   other zero is searched for; the user marks the account dormant again.

7. **Only the current episode is stored.** There is no episode history. When an
   account wakes, the months of the ended episode fall back to their own
   evidence — a statement, a per-month confirmation, or a span across them —
   and a later episode changes nothing before its own `dormant_from`. That can
   leave a month `unavailable` that was once carried at zero; it can never
   leave one `reliable` on an invented zero.

8. **Confirm unchanged follows the date.** A completed month the episode covers
   needs no confirmation and refuses one; a month before `dormant_from` follows
   the ordinary rule, previous statement required (R22, C7 unchanged).

9. **Existing rows.** A currently dormant account whose latest balance is zero
   with no attributed flow after it takes that balance's date — what marking it
   again would produce. Any other dormant account is woken. No earlier episode
   is reconstructed, and no timestamp stands in for evidence.

**A completed month with no bucket observed nothing**

10. **The worst of no statuses is not `reliable`.** 8.3 enumerates the
    currencies of a month from participating cash positions and null-leg flows.
    With neither — no cash account yet, a month before the first one opened,
    every account closed earlier — there is no bucket, and the month reported
    `reliable` with an available tracked spending of zero, which the rolling
    windows then averaged in. A user four months into tracking was shown a
    twelve-month average over twelve months, eight of them zeros nobody measured.

11. **Such a month is `unavailable`.** No tracked-cash interval was observed.
    8.6 already says an empty inclusion set is not evidence and 8.7 that a span
    needs a non-empty included set; this is the same ruling for a completed
    month. No new status is introduced. A month that was observed and reconciled
    to zero is unchanged and remains a `reliable` observation, and the
    `first_balance`, missing-evidence, `estimated` and `unresolved` cases are
    untouched.

12. **Its tracked figures are unavailable, and nothing is missing.** Every 12.5
    figure that needs the tracked scope is `Unavailable` — not zero, not
    `Partial`. No missing contribution is named, because no currency's
    contribution is missing: there was no observation. `AdditionalSpending` and
    paid-by-others need no tracked cash and stay computable, shown as their own
    facts; `TotalSpending`, the savings figures and `SavingsRate` stay
    `Unavailable` (30.15 item 1).

13. **Rolling needs no new rule.** 30.15 item 5's two conditions already
    exclude the month; the calendar window stays fixed and the count reports the
    observations that qualified.

---

### 30.21 v2.1.18 — reconciliation corrective-action UX clarification

Written while the issue catalogue gained its corrective actions. 8.5 named a
suggested action for every issue and 15.3 item 8 asked for "one-click actions",
but nothing said what a click may do, where the action lives, or what the two
"prefilled" suggestions may fill in — and four of those suggestions described a
weaker interaction than the delivered product can give. Nothing below changes an
identity, a status, an issue trigger or class, an availability rule, the schema
or a phase already delivered. The decisions are recorded in ADR 0009.

1. **A click enters the correction; it never writes.** 8.5's "nothing automatic"
   and 15.3's "one-click actions" are the same rule read from two sides: one
   click opens the focused corrective interaction — a form carrying what the
   issue knows, the existing editor for the record, the Quick update modal, or
   the month and row that maintain it — and a financial record appears only when
   the user confirms what will be recorded. An action the current phase cannot
   carry out is not offered: a withdrawal, loan proceeds, an asset sale, a
   suggested liability payment and a stale valuation stay absent until their
   record types exist.

2. **Nothing stores that an issue was handled.** A blocking issue disappears
   because the source records changed and the server recomputed (5.3, 30.19).
   Advisory dismissal remains the separate presentation state of 6.2, and no
   corrective action dismisses an advisory on the user's behalf.

3. **The adjustment is an operation, not a prefilled form.** "Accept as
   adjustment" is the fallback for `unexplained_inflow` when the real record
   cannot be identified. The server recomputes the month, finds the current
   `unexplained_inflow` for that currency and writes one `income_entries` row
   itself: `kind = adjustment`, `settlement = tracked_cash`, the amount **it**
   computed, and no cash account. It refuses when the issue has gone or its
   amount has changed, so a stale view cannot record a discrepancy the month no
   longer shows. The amount is not editable in that dialog; a user who knows the
   real figure records the real record instead.

4. **Its cash leg is null, deliberately.** The residual is a bucket-level fact
   (8.2), and 8.3's per-account residuals attribute nothing. Naming an account
   would invent an attribution, move that account's residual — which decides
   `possible_missing_interest` — and clear its dormancy (8.8). The ordinary
   null-leg rule of 8.1 still applies: a participating account of that currency
   must exist.

5. **Its date is the interval's endpoint.** `end(M)` for a completed month, `D`
   for the current one (8.6). The discrepancy is measured where the interval
   ends, so that is the honest date to record it on; it is bookkeeping rather
   than a claim about when the missing event happened, and the interface says
   so. The user is not asked to invent a date.

6. **The adjustment is available for the current month.** 30.13 item 10 already
   raises `unexplained_inflow` there, and 8.6 ranks contradictory arithmetic
   above the month being unfinished. With no `D` there is no interval, no such
   issue and no adjustment.

7. **A current-month correction stays inside `[start(M), D]`.** Month-to-date
   covers flows dated on or before `D`, so a correction offered for a
   month-to-date issue is bounded by `D` rather than by today. A record
   genuinely dated later is recorded through the ordinary Monthly editors and is
   not presented as the correction.

8. **The cross-currency suggestion prefills residuals only.** 30.15 item 9 and
   30.17 item 7 already give the prefill `U2` and `X` and refuse to choose a
   cash account when more than one exists. The same holds where exactly one
   account of a currency takes part: the advisory knows two currencies and two
   residuals, not which accounts moved money or when. The date starts empty,
   both accounts start unchosen, `X2` and the month's average rate stay on
   screen as evidence and enter no field, and no fee is assumed. Several
   qualifying sources stay several suggestions in the engine's order.

9. **The interest suggestion prefills its account.** That diagnostic is
   per account, so the account is evidence rather than a guess, and the residual
   is offered as a suggested amount the user replaces with the statement's
   figure. No date is evidenced, so none is prefilled.

10. **`first_balance` need not wait for the bulk editor.** Its action is to
    enter an earlier month-end balance, and the previous month's Accounts editor
    already records exactly one. The parenthetical "(bulk editor)" named the
    tool a later slice adds for many months at once, not the only path; 8.5 now
    says both.

11. **`flow_without_cash_account` names its record.** Under its own trigger no
    cash account of that currency takes part in the month, so "choose the cash
    account" has nothing to choose until one exists. The issue therefore carries
    the identity of the record with the unattributed leg — its kind, its id and
    its financial date — and the correction is to add an account of that
    currency and review that record; attribution through the record's own row
    editor follows once an account participates. This is result metadata: the
    trigger, class, currency, amount, sums, statuses and residuals are
    unchanged.

### 30.22 v2.1.19 — financial write-coordination clarification

Written before historical correction was built, after four read-only design
passes over what it is and what it needs underneath. 15.3 named "historical
correction" without saying which edits are one; 20.3 gave every financial row an
optimistic version and two row locks, but never said when a financial *decision*
may be read relative to the write it justifies; and 6.3 let a financial delete
carry no version at all, so a delete was the one financial mutation that could
act on a record the user had never seen. Nothing below changes an identity, a
status, an issue trigger or class, an availability rule, a rounding boundary,
the schema or a phase already delivered, and none of it needs a migration. The
decisions are recorded in ADR 0010.

1. **What a historical correction is.** An **update or delete of a source
   financial fact whose financial period is completed on either its before or
   its after side**, plus a **dormancy transition whose dated episode reaches
   completed history** — `dormant_from` is a zero balance's own date (8.8,
   30.20), so starting an episode can reinterpret months already closed. Moving
   a record out of a completed month, into one, or between two is a correction
   whichever way it goes.

2. **A single historical creation is not one.** Not because it has no historical
   consequence — recording a September expense in October plainly changes
   September — but because it is a first assertion rather than a revision: there
   is no before-image, and the before → after confirmation 15.3 asks for would
   have an empty left-hand side. 30.21 already created records inside a
   completed month on those grounds. Bulk history is **not** exempt by the same
   argument: its grid saves many months at once and mixes creates, updates and
   clears, and what the user has to see there is the aggregate effect of the
   batch, so it uses the same confirmation whatever the mix.

3. **What a correction's result distinguishes.** Three things, kept apart:
   the **source scope** (which records it touches), the **period impact** (what
   each affected financial period says before and after), and the **structural
   changes** (a span appearing or disappearing, a month's status or completeness
   moving, an issue arriving or clearing). The native-currency engines are
   authoritative for every figure; reporting-currency values stay derived with
   their own per-figure availability (8.11, 12.5, 30.16).

4. **A current month and a completed month carry different impact shapes.** A
   completed month has `end(M)` and the completed-month engine; the current
   month has `D`, or no `D` at all, and the month-to-date engine (8.6). The
   two are a union, not one shape with optional fields, and a current-month
...
6. **Mutable financial evidence, named.** `positions`, `cash_accounts`,
   ...
   `expense_entries`, `transfers`, `recurring_templates`,
   `recurring_template_terms`, `recurring_template_skips` and
   `user_settings.count_additional_spending`. That last one is a financial input
   despite its address: 12.5 makes it decide whether spending paid outside
   tracked accounts reduces personal savings, so flipping it re-interprets every
   past month's `PersonalSavings` and `SavingsRate`. Its neighbours on the same
   row — timezone, locale, favourite and reporting currency, the stale-months
   thresholds, the `preferences` JSONB — are not financial evidence and are not
   serialized against financial writes.

7. **Read committed, deliberately.** The write transaction is `read committed`.
   After waiting for the mutex, the first authoritative read must see what the
   previous holder committed, and a repeatable-read snapshot taken at the
   transaction's first statement — before the wait — would not. While the mutex
   is held no participating financial writer of the same user can commit, so the
   financial world is stable across statements regardless. No serializable
   isolation and no predicate-locking design.

8. **Reference dependencies.** Mutable eligibility state that decides whether a
   proposed financial write is *allowed* without reinterpreting existing
   history. Currently one member: `categories.archived_at`. A financial write
   that chooses a category **afresh** reads it `FOR SHARE` inside its own
   transaction and refuses a missing or archived one; `FOR SHARE` conflicts with
   the archive's `UPDATE`, so once a financial transaction has accepted a
   category as live it cannot become archived until that transaction ends.
   Category administration is **not** put on the per-user financial mutex. In
   the opposite ordering the archive commits first, the locking read then sees
   the archived row, and the write is refused with `VALIDATION_ERROR` having
   written nothing.

9. **Carrying history requires no such lock.** A historical row keeps being
   classified by its archived category's immutable `kind` (7.4, R12). Correcting
   an expense without changing its category, materializing an occurrence of a
   template whose category was live when it was chosen, and every reconciliation,
   reporting and spending read of existing expenses therefore do **not** start
   requiring a live category. Archiving a category must never rewrite the past.
...
11. **`WRITE_BUSY`.** A financial write waits for its mutex under a
    ...
    answers `WRITE_BUSY` (20.2) having written nothing. It is a benign
    retryable conflict, not an internal failure: it leaks no database
    terminology and produces no internal-error reference for ordinary
    contention.

12. **Historical confirm, when it is built.** It commits inside the same
    `read committed` mutex-owned transaction every financial mutation uses, and
    re-derives there the impact fingerprint it showed. Freshness comes from the
    statement snapshots taken after the mutex is acquired; stability comes from
    the mutex. Its preview reads one coherent `repeatable read`, `read only`
    transaction, takes no write mutex and blocks no writer. Confirm's outcome is
    a typed protocol result — committed, or the impact changed — and not an
    error code: "the world moved while you were reading the preview" is a normal
    outcome of a two-step ceremony, not a failure to log and count.

13. **What 21.5 no longer expects.** The end-to-end historical-correction
    journey listed a *restore*. Restoring a deleted financial record belongs to
    the later history-drawer and undo work, which no phase has defined; the
    journey keeps its edit, its affected months and its audit. `opened_on`
    correction, and reopening or correcting a close, likewise remain separate
    known gaps: both change which months an account participates in, and neither
    has a stated rule yet.

---

## Ready for Phase 0

No genuine blockers remain. The blueprint was frozen as v2.1.2 and Phase 0 began from it; it is frozen as v2.1.19 after the corrections in 30.6, 30.7, 30.8, 30.9, 30.10, 30.11, 30.12, 30.13, 30.14, 30.15, 30.16, 30.17, 30.18 and 30.19, none of which changed a phase already delivered, the correction in 30.20, which dates the dormant carry Phase 2 delivered and stops a completed month with no bucket from reading as a reliable zero, the clarification in 30.21, which says what a corrective action may do and gives four of 8.5's suggestions the interaction the product can actually offer, and the clarification in 30.22, which says what a historical correction is and requires every mutation of mutable financial evidence to be one atomic per-user transaction that locks before it reads.
...
- The Monthly editor has one financial calculation path: authoritative figures are recomputed on the server after a mutation commits and the page refreshes from that result; the browser validates input but calculates no figure; simple fields autosave, and a multi-field financial aggregate may save atomically with an explicit Save (4.2, 15.3, 30.19).
- Absence of evidence is never a confident figure: a dormant carry applies only from the dated start of the current episode, which must rest on a zero balance no attributed flow post-dates, and a completed month with no bucket is `unavailable` with unavailable tracked figures rather than a reliable zero (6.2, 8.1, 8.4, 8.8, 12.5, 30.20).
- A suggested action is an interaction a user can take, not a button that writes: one click enters the correction, the record is written on confirmation, the adjustment's own facts are the server's, a suggestion prefills only what it evidences, and an issue is resolved by its source records rather than by a stored flag (8.5, 15.3, 30.21).
- No remaining blocker was found. The blueprint is frozen as v2.1.18.
