...
| RecurringTemplateSkip | (explicitly skipped suggestion) | source | insert/update/delete with audit | one row per skipped occurrence of a template with a reason; the only source of occupancy facts. Accepted occurrences are the actual flow rows carrying `template_id`. |
...
- **Recurring template aggregate** = template + terms. Materializing a suggestion creates a flow row with `template_id` set; the flow is then independent.
...
| valuations, income, expenses, transfers, payments | Delete | hard delete + audit before-image; deleting a transfer cascades its fee entry |
| categories | Archive | `archived_at`; `NO ACTION` FK from expense entries blocks deletion while referenced |
| templates | Archive | `archived_at`; entries keep `template_id`; skips cascade with a hard-deleted unreferenced template |
...
Month status = worst across buckets (`unavailable > unresolved > estimated > reliable`); the reporting-currency total is `partial` when any bucket is `unavailable`. A dormant account never affects status but is listed in the metadata ("dormant, carried at 0").
...
| `mtd_no_common_date` | current month: no date in the month on which every participating non-dormant cash account has a snapshot | blocking for MTD only (MTD `unavailable`; balances still shown) | Update all cash accounts today (Quick update, all accounts pre-selected) |
...
| `suggested_income_missing` | active income template due in M with no entry and no `recurring_template_skips` row for the occurrence | advisory | Accept suggestion (date ≤ today); Skip this occurrence (writes a `recurring_template_skips` row; rental templates ask for a reason) |
...
- **Dormant account**: carried at 0 without confirmation; any non-zero balance or attributed flow clears `is_dormant` and the month then needs a month-end balance.
...
| Each active income template due in M | entry present or explicitly skipped |
...
| Bulk history editor | Reconstruct history | grid: completed months × (positions' month-end values, income per template, known expense total) | Paste/enter, save batch | grid | Blank grid from a chosen start | Carried cells greyed; current month row disabled | Desktop-first |
...
2. **Income**: one row per due template ("Salary — suggested €2,100 on 25 Sep — Accept · Edit · Skip"). Suggestions dated after today are shown as "upcoming" and cannot be accepted until their date (or "received today" re-dates them to today). Editing a suggested amount offers "This month only" or "From this month on". Skipping any suggestion writes a `recurring_template_skips` row for that occurrence; for rental income it asks for a reason (vacant / tenant did not pay / other), which is the only occupancy fact the app records.
...
**Bulk history**: grid columns = each position (month-end value, `month_end` precision), each income template (net), "known tracked expenses (total)"; rows = completed months from a chosen start (the current month row is disabled); paste from a spreadsheet (TSV, locale-aware numbers); per-cell validation; batch save in one transaction.
...
- Accepting a suggestion twice: client disables the control; the service rejects a second accepted entry for the same `(template_id, date)` with `CONFLICT_DUPLICATE`.
...
One request = a handful of bulk, index-backed queries by `user_id` and date range, then in-memory computation in `finance`: `loadFinancialWindow(ctx, from, to)` loads positions, valuations (range), flows (range), terms and the FX table once. Total and financial net worth, spans and both decompositions are produced in the same pass. Dashboard and monthly pages load ≈ 13 months; analytics "All" loads everything (≈ 30k rows ≈ 200 ms target); deterministic projections compute ≈ 360 months in < 1 s and are cached per revision.
...
- **Functionality:** income sources (templates + terms) and entries; categories UI; known expenses with how they were paid (tracked account / by me outside tracked accounts / by someone else); cash↔cash transfers (incl. cross-currency with a linked fee entry); monthly editor sections Overview, Income, Known expenses, Accounts (completed vs current month behavior), Reconciliation; suggestions (accept when dated ≤ today / upcoming / this month only / from now on / skip, with a reason for rental income); issues with one-click actions; month review state (completed months only); Spending page with tracked spending, additional spending, total spending, paid-by-others (informational), spans, saved from income and savings rate (12.5); Income page v1; bulk history editor v1 (completed months).
- **Schema:** `recurring_templates`, `recurring_template_terms`, `recurring_template_skips`, `income_entries`, `expense_entries` (with `transfer_id`), `transfers` (`cash_transfer` only), `month_reviews`.
...
| 7 | Drivers = ΔNW for both metrics and allocation identity on all fixtures and random data; non-financial purchase line; completeness text; restore from history; export scoped; first restore drill logged. |
