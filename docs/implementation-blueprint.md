...
4. **A structural zero needs the episode to cover the date.** At any date `d` —
   ...
   permitted only when `dormant_from ≤ d`. Before it the ordinary
   `carried`/`missing` rules apply, exactly as for an account that is not
   dormant, and completeness counts the account as a requirement. Where the
   latest balance on or before `d` is missing or non-zero the ordinary rules
   apply as well: the write rules below make that state unreachable, and an
   engine that met it anyway must ask for evidence rather than supply a zero.
